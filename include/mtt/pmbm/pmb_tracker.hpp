/**
 * @file      pmb_tracker.hpp
 * @brief     Wrapper class for the GM-PMB filter
 * @author    @vojtagart
 * @date      10/03/2026
...
struct PmbConfig {
    Scalar PG = 0.9999;
    Scalar min_hypot_w = 1e-5;
    Scalar min_bern_r = 1e-5;
    Scalar min_poiss_w = 1e-4;
    size_t max_hypots = 200;
    Scalar conf_thr = 0.5;
    bool recyclate = true;
    bool track_oriented = true;
    Scalar alpha = 0.0;
    bool sparsify = true;
    size_t max_per_row = 25;
};
...
struct PmbTracker {
    ...
    PmbConfig<Scalar> config;

    UpdateWorkers<Scalar, SDIM, MDIM> update_workers;
    PruneWorkers<Scalar, SDIM> prune_workers;
    PmbProjectionWorkers<Scalar, SDIM> projection_workers;

    bool t0 = true;

    /**
     * @brief Initalizes PmbTracker with given parameters
     * 
     ...
     * @param config Config for the tracker
     * @param _births0 Override of the initial birth mixture
     */
    PmbTracker(const GMT& births, const StateCovT& F, const StateCovT& Q, const HT& H, const MeasCovT& R,
            Scalar PD, Scalar PS, Scalar lambda, PmbConfig<Scalar> config, std::optional<GMT> _births0 = {})
            : births(births), births0(std::move(_births0)), dummy_mb(F.rows()), poiss(F.rows()), mbm(F.rows()), F(F), Q(Q), H(H), R(R), PD(PD), PS(PS), lambda(lambda),
              config(std::move(config)), update_workers(H.rows()) {
        ...
        assert(0 <= config.alpha && config.alpha <= 1);
        assert(config.alpha == 0.0 || !config.track_oriented);
    }

    /**
     * @brief Performs the whole PMB recursion
     * 
    ...
    /**
     * @brief Performs PMB Prediction
     */
    void prediction() {
        if (t0 && births0.has_value()) {
            pmbm::prediction(poiss, mbm, births0.value(), dummy_mb, PS, F, Q);
        } else {
            pmbm::prediction(poiss, mbm, births, dummy_mb, PS, F, Q);
        }
        t0 = false;
    }

    /**
     * @brief Performs PMB Update
     * 
     * @param Z Matrix of measurements
     */
    template <typename Derived>
    void update(const Eigen::MatrixBase<Derived>& Z) {
        ...
        pmbm::update(poiss, mbm, Z, PD, config.PG, lambda, config.max_hypots, H, R, update_workers, config.sparsify, config.max_per_row);
        pmbm::pmb_projection(mbm, config.track_oriented, projection_workers, config.alpha);
    }

    /**
     * @brief Performs PMB Pruning
     */
    void prune() {
        pmbm::prune(poiss, mbm, config.min_hypot_w, config.min_bern_r, config.min_poiss_w, prune_workers,
                    config.recyclate, false, 0.0);
    }
...
template <typename Scalar, int SDIM=Eigen::Dynamic, int MDIM=Eigen::Dynamic>
using PmbTracker = pmbm::PmbTracker<Scalar, SDIM, MDIM>;

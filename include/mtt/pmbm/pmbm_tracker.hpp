/**
 * @file      pmbm_tracker.hpp
 * @brief     Wrapper class for the GM-PMBM filter
 * @author    @vojtagart
 * @date      1/03/2026
...
struct Config {
    ...
    Scalar conf_thr = 0.5;
    bool recyclate = true;
    bool merge_comps = true;
    Scalar merge_thr = 0.0;
    bool sparsify = true;
    size_t max_per_row = 25;
};

/**
 * @brief Wrapper for the PMBM recursion
 * 
...
struct Tracker {
    ...
    Tracker(const GMT& births, const StateCovT& F, const StateCovT& Q, const HT& H, const MeasCovT& R,
            Scalar PD, Scalar PS, Scalar lambda, Config<Scalar> config, std::optional<GMT> _births0 = {})
            : births(births), births0(std::move(_births0)), dummy_mb(F.rows()), poiss(F.rows()), mbm(F.rows()), F(F), Q(Q), H(H), R(R), PD(PD), PS(PS),
              lambda(lambda), config(std::move(config)), update_workers(H.rows()) {
    ...
    /**
     * @brief Performs the whole PMBM recursion
     * 
    ...
    /**
     * @brief Performs PMBM Prediction
     */
    void prediction() {
...
template <typename Scalar, int SDIM=Eigen::Dynamic, int MDIM=Eigen::Dynamic>
using PmbmTracker = pmbm::Tracker<Scalar, SDIM, MDIM>;
